...
| selftest | 107 项（在线全跑 107/107；离线 106 PASS + 1 SKIP），分组覆盖包结构→反例→transition→DAG语义→引擎完整性→refreeze→交付缺陷回归→创建守门→日检→安装→编译→执行→版本锁定→闸门→站点一致性→长周期恢复链→并发写保护→引擎 CLI 契约→计数联动 |

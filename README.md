...
| selftest | 99 项（在线全跑 99/99；离线 98 PASS + 1 SKIP），16 个分组覆盖包结构→反例→transition→DAG语义→引擎完整性→refreeze→交付缺陷回归→创建守门→日检→安装→编译→执行→版本锁定→闸门→站点一致性→长周期恢复链→计数联动→并发写保护 |
| 总代码行（脚本） | 6,474 行 Python + Shell（scripts/ 下 wc -l 实测 2026-09-23，v1.8.14） |
| 总文档行 | 6,205 行 Markdown（docs 5,474 + 根 README 731，wc -l 实测 2026-09-23，v1.8.14） |

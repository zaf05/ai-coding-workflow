...
| selftest | 81 项（在线全跑 81/81；离线 80 PASS + 1 SKIP），14 个分组覆盖包结构→反例→transition→DAG语义→引擎完整性→refreeze→日检→安装→编译→执行→版本锁定→闸门→站点一致性→长周期恢复链 |
